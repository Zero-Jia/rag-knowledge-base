...
ADVANCED_RETRIEVAL_CACHE_VERSION = "hybrid_rerank_v3_hierarchical"
...
def _build_adv_search_cache_key(
    *,
    ...
    q = (query or "").strip()
    raw = f"v={ADVANCED_RETRIEVAL_CACHE_VERSION}|user={user_id}|mode={mode}|topk={top_k}|rm={recall_multiplier}|q={q}"
...
def retrieve_with_rerank(
    ...
    *,
    ...
    if cached is not None:
        ...
        record_rerank_scores(trace, cached if isinstance(cached, list) else [])
        return cached
    ...
    record_timing(trace, "rerank_ms", elapsed_ms(rerank_start))
    record_rerank_scores(trace, reranked)
